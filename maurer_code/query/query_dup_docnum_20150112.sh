...
cd $MY_PATH
FNAME='query_dup_docnum_20150112'
...
sales_ext AS (
	...
	LEFT OUTER JOIN
		...
		GROUP BY
			mm_fips_muni_code,
			sr_date_transfer,
			sr_doc_nbr_fmt) AS grp
	ON
		grp.mm_fips_muni_code = s.mm_fips_muni_code
		AND grp.sr_date_transfer = s.sr_date_transfer
		AND grp.sr_doc_nbr_fmt = s.sr_doc_nbr_fmt
)
		
SELECT 
	*,
	(CASE 
		WHEN price_count = 1 THEN sr_val_transfer*dup_count/grp_sqft
		WHEN price_count > 1 THEN sr_val_transfer/sa_sqft
	END) AS val_sqft
FROM 
	sales_ext
WHERE
